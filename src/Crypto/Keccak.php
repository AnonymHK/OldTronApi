...
abstract class Keccak {
	...
	static private $keccakf_rotc = [1,3,6,10,15,21,28,36,45,55,2,14,27,41,56,8,25,43,62,18,39,61,20,44];
	static private $keccakf_piln = [10,7,11,17,18,3,5,16,8,21,24,4,15,23,19,13,12,2,20,14,22,9,6,1];
	static private $x64 = (PHP_INT_SIZE === 8);

	static private function keccakf64(array $st,int $rounds) : array {
		$keccakf_rndc = [
	...
	static private function keccak64(string $in_raw,int $capacity,int $outputlength,int $suffix,bool $raw_output) : string {
		$capacity /= 8;
		...
		for($in_t = 0;$inlen >= $rsiz;$inlen -= $rsiz,$in_t += $rsiz):
			...
			$st = self::keccakf64($st,self::KECCAK_ROUNDS);
		endfor;
		$temp = mb_substr($in_raw,(int) $in_t,(int) $inlen,self::ENCODING);
		$temp = str_pad($temp,(int) $rsiz,chr(0),STR_PAD_RIGHT);
	...
	static private function keccakf32(array $st,int $rounds) : array {
		$keccakf_rndc = [
	...
	static private function keccak32(string $in_raw,int $capacity,int $outputlength,int $suffix,bool $raw_output) : string {
		$capacity /= 8;
		$inlen = mb_strlen($in_raw,self::ENCODING);
		$rsiz = 200 - 2 * $capacity;
		$rsizw = $rsiz / 8;
		...
		for($in_t = 0;$inlen >= $rsiz;$inlen -= $rsiz,$in_t += $rsiz):
			...
			$st = self::keccakf32($st,self::KECCAK_ROUNDS);
		endfor;
		$temp = mb_substr($in_raw,(int) $in_t,(int) $inlen,self::ENCODING);
		$temp = str_pad($temp,(int) $rsiz,chr(0),STR_PAD_RIGHT);
		$temp = substr_replace($temp,chr($suffix),$inlen,1);
	...
	static private function keccak(string $in_raw,int $capacity,int $outputlength,int $suffix,bool $raw_output) : string {
	...
	static public function hash(string $in,int $mdlen,bool $raw_output = false) : string {
	...
	static public function shake(string $in,int $security_level,int $outlen,bool $raw_output = false) : string {
